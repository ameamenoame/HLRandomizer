...
class ManagerCmd(Cmd):
    ...
    try:
        ...
        input(
            "Create a file named 'hlddir.txt' and place HLD installation directory inside."
        )
        exit()
    ...
    def do_gen(self, args):
        ...
        output_folder_name = (
            re.sub(r"[^\w_-]", "", input("Output folder name: ")) if output else ""
        )
        ...
        while True:
            try:
                main(
                    ...
                    output=output,
                    output_folder_name=(
                        output_folder_name if output_folder_name else "out"
                    ),
